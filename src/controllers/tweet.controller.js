...
export const getUserTweets = promiseAsyncHandler(async (req, res) => {
    ...
    // To be implemented: Fetch tweets by username
    if( !userName ) throw new ApiError(500, "Unable to identify user");

    const tweets = await Tweet.find({owner: userName?._id}).sort({createdAt: -1}).lean();
